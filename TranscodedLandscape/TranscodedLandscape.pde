/**
 * Extruded Image
 * Adapted from Transcoded example in 
...
void setup() {
  ...
  img = loadImage("rebecca-scheinberg-itsnicethat-3.jpg");
  img.loadPixels();
...
void draw() {
  ...
  // Update the angle
  float angleX = radians(map(mouseX, 0, 724, -180, 180));
  float zoom = map(mouseY, 0, 1086, 1, 4);
  rotateY(angleX);  
  scale(zoom);                        // Scale 
